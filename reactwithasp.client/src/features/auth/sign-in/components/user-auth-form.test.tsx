...
describe('UserAuthForm', () => {
  describe('Rendering without redirectTo', () => {
    ...
    beforeEach(async () => {
      vi.clearAllMocks()
      await render(<UserAuthForm />)
      emailInput = page.getByRole('textbox', { name: /^Email$/i })
      passwordInput = page.getByLabelText(/^Password$/i)
      signInButton = page.getByRole('button', { name: /^Sign in$/i })
      forgotPasswordLink = page.getByText(/^Forgot password\?$/i)
    })
    ...
    it('shows validation messages when submitting empty form', async () => {
      ...
      await expect
        .element(page.getByText(FORM_MESSAGES.emailEmpty))
        .toBeInTheDocument()
      await expect
        .element(page.getByText(FORM_MESSAGES.passwordEmpty))
        .toBeInTheDocument()
    })
